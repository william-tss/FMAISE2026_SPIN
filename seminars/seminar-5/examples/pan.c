...
short src_ln0 [] = {
	 ...
	 23,  24,  25,  17,  27,  27,  29,  30, 
	 28,  32,  32,  15,  34,  15,  34,   0, };
S_F_MAP src_file0 [] = {
	{ "-", 0, 0 },
	{ "leader.pml", 1, 22 },
	{ "-", 23, 24 }
...
{	int j = 0, h = now._nr_pr;
	...
	case 0:	/* node */
		...
		((P0 *)pptr(h))->my_pos = par1;
		/* locals: */
		((P0 *)pptr(h))->neighbor_id = 0;
		((P0 *)pptr(h))->next_chan = ((((int)((P0 *)_this)->my_pos)+1)%10);
#ifdef VAR_RANGES
		logval("node:my_id", ((P0 *)pptr(h))->my_id);
		logval("node:my_pos", ((P0 *)pptr(h))->my_pos);
		logval("node:neighbor_id", ((P0 *)pptr(h))->neighbor_id);
		logval("node:next_chan", ((P0 *)pptr(h))->next_chan);
#endif
#ifdef HAS_CODE
		locinit0(h);
#endif
...
iniglobals(int calling_pid)
{
		...
		now.q[5] = addqueue(calling_pid, 6, 0);
		now.q[6] = addqueue(calling_pid, 7, 0);
		now.q[7] = addqueue(calling_pid, 8, 0);
		now.q[8] = addqueue(calling_pid, 9, 0);
		now.q[9] = addqueue(calling_pid, 10, 0);
#ifdef VAR_RANGES
...
#endif
	switch (n) {
	case 10: j = sizeof(Q10); q_flds[10] = 2; q_max[10] = 10; break;
	case 9: j = sizeof(Q9); q_flds[9] = 2; q_max[9] = 10; break;
	case 8: j = sizeof(Q8); q_flds[8] = 2; q_max[8] = 10; break;
	case 7: j = sizeof(Q7); q_flds[7] = 2; q_max[7] = 10; break;
	case 6: j = sizeof(Q6); q_flds[6] = 2; q_max[6] = 10; break;
...
{	/* int i; */
	...
	c_chandump(now.q[4]);
	printf("	chan q[5] (=%d):	len %d:\t", now.q[5], q_len(now.q[5]));
	c_chandump(now.q[5]);
	printf("	chan q[6] (=%d):	len %d:\t", now.q[6], q_len(now.q[6]));
	c_chandump(now.q[6]);
	printf("	chan q[7] (=%d):	len %d:\t", now.q[7], q_len(now.q[7]));
	c_chandump(now.q[7]);
	printf("	chan q[8] (=%d):	len %d:\t", now.q[8], q_len(now.q[8]));
	c_chandump(now.q[8]);
	printf("	chan q[9] (=%d):	len %d:\t", now.q[9], q_len(now.q[9]));
	c_chandump(now.q[9]);
...
{	/* int i; */
	...
	printf("	byte   my_id:	%d\n", ((P0 *)pptr(pid))->my_id);
	printf("	byte   my_pos:	%d\n", ((P0 *)pptr(pid))->my_pos);
	printf("	byte   neighbor_id:	%d\n", ((P0 *)pptr(pid))->neighbor_id);
	printf("	byte   next_chan:	%d\n", ((P0 *)pptr(pid))->next_chan);
		break;
	}
}
void
printm(int x, char *s)
{
	...
	switch (x) {
	case 1: Printf("winner"); return;
	case 2: Printf("election"); return;
